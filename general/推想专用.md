...
# 代理
>`export` `http_proxy=http:``//192``.168.110.205:3128
# md5
...
d)~/Infervision/doge/docker-compose.matrix-dev.yml（应该是services/whitesharks.yml）
...
# 解决图像压缩问题（gdcm）
import os  
in_dir = "/media/tx-deepocean/Data/work/starlab/h_keypoint/example/data/input/dcm"  
  
res = []  
for f in os.listdir(in_dir):  
    f1 = os.path.join(in_dir, f)  
  
    os.system("gdcmconv -w " + f1 + " " + f1)  
    if f not in res:  
        res.append(f)  
...
sudo rm -rf log* lifenotebook-db
docker-compose up -d
并且在DS目录下
docker-compose dow -v
docker-compose up -d 

1.2、如果不需要清前端数据：
docker stop doge_ct_chest_vr_1 doge_ct_chest_vr_dual_1 && docker rm doge_ct_chest_vr_1 doge_ct_chest_vr_dual_1  && docker-compose  up -d

2.1、进数据库检查是否正常：
docker exec -it doge_repacs-db_1  mysql -upacs -p#InferShield2020# redb
select * from task
...
3、部分容器可能需要重启：
docker stop doge_repacs_1 doge_repacs-db_1 && docker restart  doge_repacs_1     doge_repacs-db_1
docker restart doge_http_server_1      doge_http_server_h_1    doge_http_server_reg_1

4、上10002端口设置DS

5、发图看看是否正常
storescu -aet TX_RETRY -aec TXPACS +sd -nh +r -d 172.16.1.7 11111 ./*

# docker 删掉没有运行的镜像
docker image prune -a

# 删除部署的数据
curl -H 'Content-Type:application/json' -X DELETE -d '{"studyInstanceUID":"1.2.840.113704.1.111.656.1433120239.1"}' [http://172.16.1.6:3333/studies/imgclean](http://172.16.1.6:3333/studies/imgclean)